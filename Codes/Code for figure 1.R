...
a = ggplot() + 
  ...
  scale_color_manual(values = c("orangered1","chartreuse2","chartreuse3","chartreuse4"))+theme_bw() +theme_classic() + theme(legend.position="none") +
...
b = ggplot() + 
  ...
  scale_color_manual(values = c("orangered1","grey20","azure4","azure3"))+theme_bw() +theme_classic()+theme(legend.position="none") +
  scale_x_continuous(name="Time of day (hour)", breaks=seq(5, 20, by = 1), limits = c(4.5, 20.5))+
  scale_y_continuous(name="Temperature (°C)", limits = c(10, 55))+
  geom_point(data = tpref, aes(hr_jitter, Tpref), colour="blue", size = 0.7)+
  geom_point(data = tpref_mean, aes(hr, Tpref), color="purple", size=2) 
...
c = ggplot() + xlim(7.5, 18.5) + ylim(10, 55) +
  ...
  scale_color_manual(values = c("orangered1","chartreuse2","chartreuse3","chartreuse4"))+theme_bw() +theme_classic() + theme(legend.position="none") +  
  scale_x_continuous(name="Time of day (hour)", breaks=seq(5, 20, by = 1), limits = c(7.5, 18.5))+
  scale_y_continuous(name="Temperature (°C)", limits = c(10, 55))+
  geom_point(data = tpref, aes(hr_jitter, Tpref), colour="blue", size = 0.7)+
  geom_point(data = tpref_mean, aes(hr, Tpref), color="purple", size=2) 
...
tiff("Figure 1.tif", width=2000, height = 1500, compression = "lzw", res=300)
ggarrange(a, b, c, d, 
          labels = c("A", "B", "C", "D"),
          ncol = 2, nrow = 2)
dev.off()
